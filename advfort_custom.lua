...
local keybinds={
    nextJob={key="CUSTOM_E",desc="Next job in the list"}, -- CUSTOM_SHIFT_T
    prevJob={key="CUSTOM_Q",desc="Previous job in the list"}, -- CUSTOM_SHIFT_R
    ...
    quick={key="CUSTOM_W",desc="Toggle quick item select"},
    }
    -- building filters
    local build_filter={
    ...
    local settings={
        build_by_items=false,
        use_worn=false,
        check_inv=true,
        teleport_items=true,
        df_assign=false,
        gui_item_select=true,
        only_in_sites=false,
        set_civ="MOUNTAIN"} -- was "MOUNTAIN"
    ...
    local actions={ --as:{1:string,2:df.job_type,3:'{_type:function,_node:{_type:tuple,_tuple:[bool,string]},_anyfunc:true}[]',4:'{_type:function,_node:{_type:tuple,_tuple:[bool,string]},_anyfunc:true}[]',5:'{_type:function,_node:{_type:tuple,_tuple:[bool,string]},_anyfunc:true}[]'}[]
        ...
        -- {"CarveTrack"           ,df.job_type.CarveTrack,{},{SetCarveDir}}, --TODO: check this- carving modifies standing tile but depends on direction!
    ...
    local actionChoiceList = {}
    for k, item in pairs(actions) do
        table.insert(actionChoiceList, item[1])
    end
    ...
    function usetool:onInput(keys)
        ...
        elseif keys[keybinds.nextJob.key] then --next job with looping
            mode=(mode+1)%#actions
            self.subviews.actionList:setSelected(mode+1)
